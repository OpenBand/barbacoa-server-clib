...
BOOST_AUTO_TEST_CASE(rubber_init_check)
{
    ...
    BOOST_REQUIRE(rubber_init(&ctx, chunk_sz, false) > 0);
    ...
    BOOST_REQUIRE(rubber_destroy(&ctx) > 0);
}
...
BOOST_AUTO_TEST_CASE(rubber_init_from_buff_check)
{
    ...
    BOOST_REQUIRE(rubber_init_from_buff(&ctx, info, sizeof info, chunk_sz, false) > 0);
...
BOOST_AUTO_TEST_CASE(rubber_go_whithout_enlarge_check)
{
    ...
    int wrn = 0;
    BOOST_REQUIRE(rubber_pos(&ctx, &wrn));
    wrn = 5;
    BOOST_REQUIRE(rubber_pos(&ctx, &wrn));
