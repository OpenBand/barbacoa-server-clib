...
{
    uint32_t L = 1, R = 2;
    srv_c_blowfish_ctx_t ctx;

    srv_c_blowfish_init(&ctx, (uint8_t*)"TESTKEY", 7);
    srv_c_blowfish_encrypt_chunk(&ctx, &L, &R);
    BOOST_REQUIRE(L == 0xDF333FD2L);
    BOOST_REQUIRE(R == 0x30A71BB4L);

    srv_c_blowfish_decrypt_chunk(&ctx, &L, &R);
    BOOST_REQUIRE(L == 1);
    BOOST_REQUIRE(R == 2);
}
...
BOOST_AUTO_TEST_CASE(chunk_encryption_check)
{
    ...
    BOOST_REQUIRE(srv_c_blowfish_init(&ctx, (uint8_t*)key, sizeof(key)));
    BOOST_REQUIRE(srv_c_blowfish_encrypt_chunk(&ctx, &L, &R));
    BOOST_REQUIRE(srv_c_blowfish_destroy(&ctx));
    BOOST_REQUIRE_EQUAL(ctx.P[0], 0);
    BOOST_REQUIRE_EQUAL(ctx.S[0][0], 0);

    uint32_t enc_L = L, enc_R = R;

    BOOST_REQUIRE(srv_c_blowfish_init(&ctx, (uint8_t*)wrong_key, sizeof(wrong_key)));
    BOOST_REQUIRE(srv_c_blowfish_decrypt_chunk(&ctx, &L, &R));
...
BOOST_AUTO_TEST_CASE(data_encryption_check)
{
    ...
    auto enc_sz = srv_c_blowfish_get_stream_output_length(sizeof(input_data));
    BOOST_REQUIRE_GE(enc_sz, sizeof(input_data));
    BOOST_REQUIRE_EQUAL(enc_sz % srv_c_blowfish_get_min_chunk_length(), 0);

    unsigned char* enc_data = (unsigned char*)alloca(enc_sz);
    BOOST_REQUIRE(srv_c_blowfish_init(&ctx, (uint8_t*)key, sizeof(key)));
    BOOST_REQUIRE_EQUAL(
        srv_c_blowfish_stream_encrypt(&ctx, (const unsigned char*)input_data, sizeof(input_data), enc_data, enc_sz), enc_sz);
    BOOST_REQUIRE(srv_c_blowfish_destroy(&ctx));

    BOOST_REQUIRE(srv_c_blowfish_init(&ctx, (uint8_t*)key, sizeof(key)));
    unsigned char* dec_data = (unsigned char*)alloca(enc_sz);
    BOOST_REQUIRE_EQUAL(srv_c_blowfish_stream_decrypt(&ctx, enc_data, enc_sz, dec_data, enc_sz), enc_sz);
    BOOST_REQUIRE(srv_c_blowfish_destroy(&ctx));
...
BOOST_AUTO_TEST_CASE(data_stream_encryption_check)
{
    ...
    BOOST_REQUIRE_GE(chuck_sz, srv_c_blowfish_get_min_chunk_length());

    BOOST_REQUIRE(srv_c_blowfish_init(&ctx, (uint8_t*)key, sizeof(key)));

    size_t rest_sz = sizeof(input_data);
    std::vector<unsigned char> encoded;
    encoded.reserve(srv_c_blowfish_get_stream_output_length(rest_sz));
    ...
    while (rest_sz > 0)
    {
        ...
        auto chunk_actial_out = std::max(chunk_actial_in, static_cast<size_t>(srv_c_blowfish_get_min_chunk_length()));
        auto r = srv_c_blowfish_stream_encrypt(&ctx, p_in_pos, chunk_actial_in, &chunk[0], chunk_actial_out);
...

    {
        ...
        BOOST_REQUIRE_EQUAL(srv_c_blowfish_stream_decrypt(&ctx, &encoded[0], encoded.size(), dec_data, encoded.size()),
                            encoded.size());
        BOOST_REQUIRE(srv_c_blowfish_destroy(&ctx));
    ...
    while (rest_sz > 0)
    {
        auto chunk_actial_in = std::min(chuck_sz, rest_sz);
        auto chunk_actial_out = std::max(chunk_actial_in, static_cast<size_t>(srv_c_blowfish_get_min_chunk_length()));
        auto r = srv_c_blowfish_stream_decrypt(&ctx, &encoded[ci], chunk_actial_in, &chunk[0], chunk_actial_out);
        BOOST_REQUIRE_GT(r, 0);
        ci += r;
